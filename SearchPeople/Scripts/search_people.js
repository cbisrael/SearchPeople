...
$(document).ready(function () {
    ...
    $("#fleUpload").change(function () {
        //Purpose: Call the Picture.Show to render the image the user selected on the
...
//Person class to search, qeruy for update, and update person data
var Person = {
    //output_person_records
        ...
        $("#people_power").append(
            $.map(data, function (ignore, index) {
                var _html = "<tr><td style='vertical-align: middle'>" + get_pic(data[index].pic_path) + "</td>" +
                    ...
                    "</td><td style='vertical-align: middle'><a href='#' id='" + data[index].iperson + "' onclick='Person.Query(this)'>Edit</a></tr>";
                return _html;

            }).join()
         );
    },
    Send: function (fle_name) {
    ...
    Query: function (_href) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                clear_fields();
                $("#upd_iperson").val(trim_data(data.iperson));
...
//Picture class to handle picture uploads
var Picture = {
    Upload: function () {
        //Purpose: Entry poin tof child upload.  It calls Picture.UploadChunk where UploadChunk calls peforms
    ...
    UploadChunk: function (file) {
        ...
        //          Picture.UploadComplete routine
        ...
        xhr.onload = function () {
            ...
            if (m_completed === count) {
                Picture.UploadComplete(file);
            }
            else {
                if (m_start_read < FILE_SIZE)
                    Picture.UploadChunk(file); //recursive call until the whole file is uploaded
            }
        };
    ...
    UploadComplete: function (_file) {
        //Purpose: Called when the picture is completely uploaded to the server.  It will then reset the 
        //         variables used to upload the picture file and proceed to call Person.Send to send
